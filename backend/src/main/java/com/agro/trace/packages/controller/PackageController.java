...
import com.agro.trace.packages.dto.SupplierAllotmentTransferRequest;
...
public class PackageController {
    ...
    @PostMapping("/lots/{lotId}/allotment-qr")
    public ResponseEntity<ApiResponse<String>> generateSupplierAllotmentQr(
            @PathVariable String lotId,
            Authentication authentication) {
        var qrId = packageService.generateSupplierAllotmentQr(lotId, authentication.getName());
        return ResponseEntity.ok(ApiResponse.success(qrId));
    }

    @PostMapping("/allotment/transfer")
    public ResponseEntity<ApiResponse<?>> transferPackagesWithAllotment(
            @RequestBody SupplierAllotmentTransferRequest request,
            @RequestParam(required = false) Double latitude,
            @RequestParam(required = false) Double longitude,
            Authentication authentication) {
        var response = packageService.transferWithAllotmentScan(
                request.allotmentQrId(),
                request.packageIds(),
