...
export function NodalDashboard() {
  ...
  return (
    <div>
      <PageHeader title="Nodal Center" subtitle="Incoming lots and package management" />

      {error && <ErrorBanner message={error} onRetry={fetchLots} />}
      {loading && <SkeletonList count={5} />}

      {!loading && !error && lots.length === 0 && (
        <EmptyState title="No incoming lots" description="No lots are currently at the nodal center." />
      )}
        ...
        <div>
          ...
          <div className="agri-stagger">
            {lots.map((lot) => (
              <div
                key={lot.lotId}
                onClick={() => navigate(`/nodal/lots/${lot.lotId}`)}
                className="agri-card"
