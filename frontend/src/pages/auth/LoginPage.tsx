...
export function LoginPage() {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      useAuthStore.getState().setWalletAddress(account.address, email);
      setStatus('Email verified! Enter your PIN to sign in.');
      setStep('pin');
    } catch (err) {
  ...
  const redirectToRole = (userType: string) => {
    switch (userType) {
      ...
      <div style={{ width: '100%', maxWidth: 400 }}>
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 13, color: colors.graphite, marginTop: 4 }}>
              Agricultural Supply Chain Traceability
            </p>
          </div>

          <div style={{ display: 'flex', gap: 8, marginBottom: 28, justifyContent: 'center' }}>
            {(['email', 'otp', 'pin'] as Step[]).map((s) => (
              <div key={s} style={{
                width: step === s ? 24 : 8, height: 8, borderRadius: 4,
                backgroundColor: (s === step || (s === 'otp' && step === 'pin') || (s === 'email' && step !== 'email'))
          ...
          {step === 'pin' && (
            <div style={{ animation: 'fadeInUp 0.3s ease-out both' }}>
              <p style={{ fontSize: 13, color: colors.graphite, marginBottom: 16 }}>
                Enter your 6-digit PIN to sign in to <strong>{email}</strong>
              </p>
              ...
              />
              <Button onClick={handleLogin} disabled={pin.length !== 6} fullWidth loading={loading} style={{ marginTop: 16 }}>
                Sign In
              </Button>
            </div>
          )}
